...
class simple_critic_network(critic_network):
    ...
    def __init__(self, state_size, action_size, action_bound = None, l1_size = 200, l2_size = 100, learning_rate = 0.0001):
        ...
        with self.graph.as_default():
            ...
            self.x2 = tf.nn.tanh(tf.matmul(self.x1,self.W2) + tf.matmul(self.action_input,self.W2_action) + self.b2)
            ...
            self.x2_target = tf.nn.tanh(tf.matmul(self.x1_target,self.W2_target) + tf.matmul(self.action_input,self.W2_action_target) + self.b2_target)
            ...
            self.diff = tf.pow(self.qval_output-self.qval_train, 2)/tf.to_float(tf.shape(self.qval_train)[0]) + 0.0001*tf.reduce_sum(tf.pow(self.W2,2))+ 0.0001*tf.reduce_sum(tf.pow(self.b2,2))
